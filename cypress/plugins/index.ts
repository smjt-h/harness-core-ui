...
module.exports = (on: Cypress.PluginEvents, config: Cypress.PluginConfigOptions) => {
  ...
  on('before:browser:launch', (browser, launchOptions) => {
    if (browser.name === 'chrome' && browser.isHeadless) {
      // fullPage screenshot size is 2800x2800 on non-retina screens
      // and 2800x2400 on retina screens
      launchOptions.args.push('--window-size=2800,2800')

      // force screen to be non-retina (2800x2800 size)
    ...
    if (browser.name === 'electron' && browser.isHeadless) {
      // fullPage screenshot size is 2800x2800
      launchOptions.preferences.width = 2800
      launchOptions.preferences.height = 2800
    }

    if (browser.name === 'firefox' && browser.isHeadless) {
      // menubars take up height on the screen
      // so fullPage screenshot size is 2800x1126
      launchOptions.args.push('--width=2800')
