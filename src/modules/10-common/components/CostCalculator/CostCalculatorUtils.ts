...
enum ffUnitTypes
    { DEVELOPER = 'developer', MAU = 'MAU'}
const UnitRanges = {
    developer : ['developer',1,50,1],
    mau : ['mau',25, 1000, 25]
}

const getSupportType = (planType : PlanType) => {
    switch (planType) {
...
const acquireCurrentPlan = (page: string, account? : string) => {
    if(page === 'PLANS') {
...
const productCosts = {developer : {'yearly': 180, 'monthly' : Math.round(180 / 12 / 0.8)}, MAU : {
        'yearly' : 900, 'monthly' : 900/ 12/ 0.8
}};

const getCostForUnits = (units : number, unitType: ffUnitTypes,  edition : Editions, planType : PlanType) => {
    switch (edition) {
        case Editions.FREE:
            return 0;
        case Editions.TEAM:
...
//}
export const calculateCostTotal = (unitsInUse: number, costPerUnit: number, unitsInPlan?: number, surcharge?: number) => {
