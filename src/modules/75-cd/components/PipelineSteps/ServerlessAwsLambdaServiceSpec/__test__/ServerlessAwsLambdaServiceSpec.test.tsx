...
import TemplateMock from '../../K8sServiceSpec/__tests__/template.mock.json'
import connectorListJSON from '../../K8sServiceSpec/__tests__/connectorList.json'
import secretMockdata from '../../K8sServiceSpec/__tests__/secretMockdata.json'
import { PipelineResponse } from '../../K8sServiceSpec/__tests__/pipelineMock'
import { getParams, mockBuildList, mockManifestConnector } from '../../K8sServiceSpec/__tests__/mocks'
import { getYaml } from './mocks'
...
describe('Autocomplete fields test', () => {
  test('Test connectorRef Manifest', async () => {
    ...
    list = await step.getManifestConnectorsListForYaml(connectorRefPath, yamlString, getParams())
    expect(list).toHaveLength(1)
    expect(list[0].insertText).toBe('account.git9march')
    list = await step.getManifestConnectorsListForYaml('invalid path', yamlString, getParams())
    expect(list).toHaveLength(0)
  })
  test('Test connectorRef ArtifactsPrimaryConnectors', async () => {
    const step = new ServerlessAwsLambdaServiceSpec() as any
    let list: CompletionItemInterface[]
    list = await step.getArtifactsPrimaryConnectorsListForYaml(connectorArtifactPrimaryRefPath, yamlString, getParams())
    expect(list).toHaveLength(1)
    expect(list[0].insertText).toBe('account.git9march')
    list = await step.getArtifactsPrimaryConnectorsListForYaml('invalid path', yamlString, getParams())
    expect(list).toHaveLength(0)
  })
})
