...
const GitSyncRepoForm: React.FC<ModalConfigureProps & GitSyncRepoFormProps & StepProps<GitSyncConfig & StepFormData>> =
  props => {
    ...
    const supportedProviders = bitBucketSupported
      ? gitCards
      : gitCards.filter(card => card.type !== Connectors.BITBUCKET)

    const defaultInitialFormData: GitSyncFormInterface = props.prevStepData?.formData || {
    ...
    useEffect(() => {
      if (props.prevStepData?.formData) {
        const { connector: value } = props.prevStepData?.formData?.gitConnector || {}
        if (value?.spec?.type === GitUrlType.REPO) {
          setConnectorIdentifierRef(getConnectorIdentifierWithScope(getScopeFromDTO(value), value.identifier))
          setRepositoryURL(value?.spec?.url)
        }
        setTestStatus(TestStatus.NOT_INITIATED)
      }
    }, [props.prevStepData?.formData])

    const handleCreateAndChangeStep = async (formData: GitSyncFormInterface) => {
      try {
        const data = getGitSyncDataFromFormData(formData, orgIdentifier, projectIdentifier)
        if (isNewUser) {
          props.nextStep?.({ ...props, ...data, formData, name: data.name })
          return
        }
        ...
        //use response to convert into form data;
        props.nextStep?.({ ...props, ...response, formData, name: response.name })
      } catch (e) {
        ...
        {isNewUser ? (
          <GitSyncStoreProvider>
            <SCMCheck
              ...
              title={modalTitle}
              validateSCM={hasSCM => setNeedSCM(!hasSCM)}
            />
          </GitSyncStoreProvider>
        ) : (
        ...
        <Layout.Horizontal>
          <Container width={'60%'}>
            <Formik<GitSyncFormInterface>
              ...
              onSubmit={formData => {
                handleCreateAndChangeStep(formData)
              }}
            >
