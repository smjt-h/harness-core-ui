...
import { Button, ButtonVariation, Color, FontVariation, Layout, Text, Icon, IconName } from '@wings-software/uicore'
...
interface ResourceCenterHomeParams {
  onClose: { (event: React.MouseEvent): Promise<void> | void }
  submitTicket: ((event: React.MouseEvent<Element, MouseEvent>) => void | Promise<void>) | undefined
}

export const ResourceCenterHome = ({ onClose, submitTicket }: ResourceCenterHomeParams) => {
...
export const ResourceCenter = () => {
  const rcStates = { resourceHome: 'resourceHome', submitTicket: 'submitTicket' }

  const [showRC, setShowRC] = useState(false)
  const [showState] = useState(rcStates.resourceHome)

  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const Saber = window['Saber']

  const resourceCenterBody = (
    <ResourceCenterHome onClose={() => setShowRC(false)} submitTicket={() => Saber.do('open')} />
  )

  return (
    <ModalProvider>
      {showRC && showState === rcStates.resourceHome && resourceCenterBody}
      {!showRC && <Button intent={'primary'} round icon={'question'} minimal onClick={() => setShowRC(true)} />}
    </ModalProvider>
  )
}
