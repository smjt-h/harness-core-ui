...
import { useModalHook } from '@wings-software/use-modal'
...
export function SavePipelinePopover({
  ...
  const {
    state: { pipeline, yamlHandler, gitDetails, pipelineView },
    deletePipelineCache,
    fetchPipeline,
    view,
    setSchemaErrorView,
  ...
  if (isReadonly && !isTemplatesEnabled) {
    return <></>
  }

  if (isReadonly && isTemplatesEnabled) {
    return <SaveTemplateButton type={'Pipeline'} data={pipeline} />
  }
