...
import {
  ...
  getServiceNowCreateEditModePropsWithConnectorId
...
jest.mock('services/cd-ng', () => ({
  ...
  useGetServiceNowProjects: jest.fn(),
  useGetServiceNowIssueCreateMetadata: () => mockProjectMetadataResponse
}))
describe('ServiceNow Create fetch projects', () => {
  beforeAll(() => {
    // eslint-disable-next-line
    // @ts-ignore
    useGetServiceNowProjects.mockImplementation(() => mockProjectsErrorResponse)
  })
  beforeEach(() => {
  ...
  test('show error if failed to fetch projects', () => {
    ...
    const props = getServiceNowCreateEditModePropsWithConnectorId()
...
describe('ServiceNow Create tests', () => {
  beforeAll(() => {
    // eslint-disable-next-line
    // @ts-ignore
    useGetServiceNowProjects.mockImplementation(() => mockProjectsResponse)
  })
  beforeEach(() => {
    factory.registerStep(new ServiceNowCreate())
  })
  ...
  test('Basic functions - edit stage view validations', async () => {
    ...
    await waitFor(() => {
      expect(queryByText('pipeline.ServiceNowApprovalStep.validations.project')).toBeTruthy()
      expect(queryByText('pipeline.ServiceNowApprovalStep.validations.issueType')).toBeTruthy()
    })
    await waitFor(() => expect(queryByText('pipeline.ServiceNowCreateStep.validations.summary')).toBeTruthy())
  })
  ...
  test('Open a saved step - edit stage view', async () => {
    ...
    fireEvent.change(getByPlaceholderText('pipeline.ServiceNowCreateStep.summaryPlaceholder'), {
      target: { value: 'summary' }
    })

    // Open the fields selector dialog
    act(() => {
      fireEvent.click(getByText('pipeline.ServiceNowCreateStep.fieldSelectorAdd'))
    })
    ...
    fireEvent.click(getByText('pipeline.ServiceNowCreateStep.provideFieldList'))
