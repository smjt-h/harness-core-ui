...
function ManifestListView({
  ...
  const attachPathYaml = useCallback(
    (manifestPathData: any, manifestId: string, manifestType: PrimaryManifestType): void => {
      ...
      if (manifestData.manifest.spec[ManifestToPathKeyMap[manifestType]]?.length) {
        manifestData.manifest.spec[ManifestToPathKeyMap[manifestType]].push(...manifestPathData)
      } else {
                ...
                return (
                  <div className={css.rowItem} key={`${manifest?.identifier}-${index}`}>
                    ...
                    <AttachPathYamlFlow
                      ...
                      valuesPath={manifest?.spec[ManifestToPathKeyMap[manifest?.type as PrimaryManifestType]]}
                      expressions={expressions}
                      allowableTypes={allowableTypes}
